...
public class PointOfInterestMarker : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if (TargetMarker.GetComponent<EnemyAudio> ())
		{
			TargetMarker.GetComponent<EnemyAudio> ().currentWaypoint = this.gameObject;
		}

        if(!PlayerPosition)
            PlayerPosition=Game.instance.player.transform;
	}
    ...
    private void BuildDistanceStringNoAlloc(float distance)
    {
        ...
        do
        {
            ...
            wholeVal = (wholeVal - digit)/10;//was 10
        } while (wholeVal > 0);
